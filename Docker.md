...
### 4. Run the following command to bring up the entire environment in one step (where you can see logs):
```
docker compose up
```
#### NOTES:
...
- Think of this as running 4 distinct servers, each self-contained yet able to cross-communicate, within the host machine.
	...
	- Both OCNES and Celery map the project source files into their respective containers, meaning they access the same files as the host running these containers.  So any changes made will be reflected in those containers automatically.
- The first time this command is executed, it may take a few minutes, as it has to download images from `hub.docker.com` and build a few of the container images.  Once built, however, this command should come up within seconds on successive executions.
...
- The OCNES/Django and Celery images, which are built locally for this project, will be named for the directory that `docker-compose.yml` was in when created.  (If you are an OOP programmer, think of images as classes and containers as instances of those classes.)  So you should see `dashboard-ocnes` and `dashboard-celery` as images.  And the active containers will be named `dashboard-ocnes-1` and `dashboard-celery-1`.  (Docker numbers each instance it spins up.)  This is handy to know as you can get into running containers using `docker exec`.

### 4. After the initial launch (and every subsequent time you spin up this container), the PostgreSQL container instance is already preconfigured/updated through `makemigrations` and `migrate`.  However, should you need to do any manual work with the Django admin tools, simply execute the following command, which will place you *inside* the OCNES container:

```
docker exec -it dashboard-ocnes-1 /bin/bash
```

- When you are inside a container, you should be at a `#` prompt (since you are root), and if the OCNES or Celery containers, it should place you right in the `/dashboard` directory where the OCNES project files are located.  At this point you can do the usual commands such as `python manage.py migrate`, etc.

### 5. At this point, all the pieces are running and you have all migrations done.  However, you are not done yet.  You do not yet have a super user defined that you can log into OCNES with.  So next enter the following command in the OCNES container:
...
### 6. At this point, you have the basics done.  Exit the running container by entering `exit`
### 7. Point your web browser at http://127.0.0.1:8000 and see the login page.
### 8. Enter the username and password you provided earlier, and voila!  Welcome to OCNES!
### 9. When you are finished, you can spin down the containers by pressing `[CTRL][C]` or, if you used `-d` when bringing up the containers, you can use
...
- The setup as defined has the PostgreSQL container storing all data within a hidden directory at the root of the project files; specifically in `.docker/pgdata/`.  This is to provide persistence across container restarts.  Note that if you ever want to "reset" PostgreSQL back to its initial state, simply delete this directory.
